...
impl ValidationBuilder {
    ...
    pub fn email(self) -> Self {
        self.add(ValidationOperator::email())
    }

    /// Adds URL validation
    pub fn url(self) -> Self {
        self.add(ValidationOperator::url())
    }

    /// Adds UUID validation
    pub fn uuid(self) -> Self {
        self.add(ValidationOperator::uuid())
    }
    ...
    /// Adds is empty validation
    pub fn is_empty(self) -> Self {
        self.add(ValidationOperator::IsEmpty)
    }

    /// Adds is not empty validation
    pub fn is_not_empty(self) -> Self {
        self.add(ValidationOperator::IsNotEmpty)
    }

    /// Adds is null validation
    pub fn is_null(self) -> Self {
        self.add(ValidationOperator::IsNull)
    }

    /// Adds is not null validation
    pub fn is_not_null(self) -> Self {
        self.add(ValidationOperator::IsNotNull)
    }

    /// Adds equals field validation
    pub fn equals_field(self, field: impl Into<ParameterKey>) -> Self {
        self.add(ValidationOperator::EqualsField(field.into()))
    }

    /// Adds not equals field validation
    pub fn not_equals_field(self, field: impl Into<ParameterKey>) -> Self {
        self.add(ValidationOperator::NotEqualsField(field.into()))
    }

    /// Adds greater than field validation
    pub fn greater_than_field(self, field: impl Into<ParameterKey>) -> Self {
        self.add(ValidationOperator::GreaterThanField(field.into()))
    }

    /// Adds less than field validation
    pub fn less_than_field(self, field: impl Into<ParameterKey>) -> Self {
        self.add(ValidationOperator::LessThanField(field.into()))
    }

    /// Adds conditional requirement
    pub fn required_if(self, field: impl Into<ParameterKey>, condition: ValidationOperator) -> Self {
        self.add(ValidationOperator::RequiredIf(field.into(), Box::new(condition)))
    }

    /// Adds conditional forbiddance
    pub fn forbidden_if(self, field: impl Into<ParameterKey>, condition: ValidationOperator) -> Self {
        self.add(ValidationOperator::ForbiddenIf(field.into(), Box::new(condition)))
    }

    /// Adds strong password validation
    pub fn strong_password(self) -> Self {
        self.add(ValidationOperator::strong_password())
    }

    /// Adds phone number validation
    pub fn phone_number(self) -> Self {
        self.add(ValidationOperator::IsPhoneNumber)
    }

    /// Adds IP address validation
    pub fn ip_address(self) -> Self {
        self.add(ValidationOperator::IsIpAddress)
    }

    /// Builds the final validation operator using AND logic
    pub fn build(self) -> ValidationOperator {
        match self.operators.len() {
            0 => ValidationOperator::IsNotNull, // Default: just not null
    ...
    pub fn build_or(self) -> ValidationOperator {
        match self.operators.len() {
            0 => ValidationOperator::IsNotNull,
            1 => self.operators.into_iter().next().unwrap(),
